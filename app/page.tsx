...
function ReservationForm() {
  return <form className="booking" action={`mailto:${email}`} method="get">
    <label><span>Départ</span><input name="depart" placeholder="Votre adresse" required /></label>
...
export default function Home() {
  return <main>
    ...
    <section className="intro section wrap" id="apropos">
      ...
      <div className="intro-body"><p>De votre réservation à votre arrivée, chaque étape est anticipée pour vous permettre de voyager sereinement. Votre chauffeur vous accompagne avec ponctualité, discrétion et attention.</p><a className="text-link dark" href="/a-propos">Découvrir notre approche <i>↗</i></a></div>
    </section>

    <section className="services section-dark" id="services"><div className="wrap"><div className="section-heading"><p className="eyebrow">Nos services</p><h2>Chaque trajet a son attention particulière.</h2><p>Une prise en charge précise, pensée autour de votre agenda.</p></div><div className="service-grid">{services.map(([title, copy, label], index) => <a className={`service-card service-${index + 1}`} href={index === 1 ? "/transfert-aeroport" : index === 2 ? "/transfert-gare" : index === 3 ? "/evenements" : "/chauffeur-prive-arras"} key={title}><div><span>{label}</span><b>0{index + 1}</b></div><h3>{title}</h3><p>{copy}</p><i>↗</i></a>)}</div></div></section>

    <section className="airport section" id="transferts"><div className="airport-photo" /><div className="airport-copy"><p className="eyebrow ink">Transferts aéroports</p><h2>Votre vol. Votre chauffeur. Aucun stress entre les deux.</h2><p>Nous organisons votre départ ou votre retour en tenant compte de vos horaires et de vos bagages.</p><div className="route-list"><span>Lille-Lesquin</span><span>Paris Charles-de-Gaulle</span><span>Paris-Orly</span><span>Beauvais</span></div><a className="text-link dark" href="/transfert-aeroport">Préparer mon transfert <i>↗</i></a></div></section>

    <section className="vehicle section-dark"><div className="wrap vehicle-grid"><div><p className="eyebrow">À bord</p><h2>Un espace pensé pour le trajet.</h2><p className="muted">Le confort n’est pas une promesse abstraite : c’est l’espace, le calme et l’attention qui vous permettent d’arriver serein.</p><ul className="features"><li>Confort arrière</li><li>Climatisation</li><li>Chargeurs à bord</li><li>Eau à disposition</li></ul><p className="fine-print">Équipements proposés selon le véhicule et la prestation. Contactez-nous pour une demande spécifique.</p><a className="text-link" href="/vehicule">Découvrir le véhicule <i>↗</i></a></div><div className="vehicle-photo" /></div></section>

    <section className="chauffeur section wrap"><div className="driver-photo" /><div className="driver-copy"><p className="eyebrow ink">Votre chauffeur</p><h2>Une exigence simple : que votre trajet se déroule parfaitement.</h2><p>Connaissance de la région, conduite souple, discrétion et disponibilité : une présence attentive, sans jamais être intrusive.</p><a className="text-link dark" href="/a-propos">En savoir plus <i>↗</i></a></div></section>

    <section className="reviews section" id="avis"><div className="wrap"><div className="review-title"><p className="eyebrow ink">Avis clients</p><h2>La confiance se construit trajet après trajet.</h2><div className="stars">★★★★★ <span>Votre avis compte</span></div></div><div className="review-grid"><blockquote><div>★★★★★</div><p>« Un trajet très agréable, ponctuel et professionnel. Une vraie tranquillité d’esprit pour un départ tôt le matin. »</p><footer>Client VTC ARRAS HDF <span>· Google</span></footer></blockquote><blockquote><div>★★★★★</div><p>« Réservation simple, chauffeur prévenant et voiture très confortable. Je recommande pour les transferts gare. »</p><footer>Client VTC ARRAS HDF <span>· Google</span></footer><a className="text-link dark" href={`mailto:${email}?subject=Votre%20avis`}>Laisser un avis <i>↗</i></a></blockquote></div></div></section>

    <section className="pro section-dark" id="pro"><div className="wrap pro-grid"><div><p className="eyebrow">Professionnels</p><h2>Vos déplacements professionnels, sans imprévu.</h2></div><div><p>Un interlocuteur fiable pour vos rendez-vous, vos transferts gare ou aéroport et les déplacements de vos collaborateurs.</p><div className="pro-points"><span>Ponctualité</span><span>Confidentialité</span><span>Réservation anticipée</span><span>Facturation professionnelle</span></div><a className="button button-outline" href="/professionnels">Parler de vos besoins <i>↗</i></a></div></div></section>
